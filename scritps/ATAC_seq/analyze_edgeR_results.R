...
for (norm_method in names(norm_method_list)) {
  ...
  fit <- readRDS("composition_fit.RDS")
...
write.table(bed_filtered_peaks,"../../data/filtered_windows.bed",sep = "\t",quote = F,row.names = F,col.names = F)
#plotMD(fit120vs0)

#chequear normalizacion (manual csaw) tanto metodo como el hecho de si tiene en cuenta coverage en al menos una muestra (pregunta de Sergi)
#generar bed file con filtered windows 
#