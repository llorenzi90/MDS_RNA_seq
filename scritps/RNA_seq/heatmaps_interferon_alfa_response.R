...
#read in interferon alfa genes
intfa <- read.table("~/shares/INVESTIGACIO/Cuartero Group/CUARTERO GROUP/references/human/gene_sets/TSS_files/HALLMARK_INTERFERON_ALPHA_RESPONSE.TSSs.txt",header = T)
...
showleg=T 
geneset="HALLMARK INTERFERON ALPHA RESPONSE"

pdf("default_pheatmap_Colors.pdf")
ComplexHeatmap::pheatmap(as.matrix(data_to_plot),cluster_rows = T,
                         cluster_cols = F,
                         show_colnames = T,
                        fontsize_col = 4,
                        angle_col=c("45"),
...
#let's check this
hm=draw(hm)
gene_order_after_Clustering <- rownames(data_to_plot)[row_order(hm)]
...
library(ggplot2)
ifrnagenes=deseq2res[match(gene_order_after_Clustering,deseq2res$gene_name),]
...
rownames(annorowdf) <- ifrnagenes$gene_name
#order must match the order of the input matrix:
annorowdf <- annorowdf[match(rownames(data_to_plot),rownames(annorowdf)),]
...
pdf(paste0("manual_scaling_",pname,"_colors.rowAnnot.pdf"))
ComplexHeatmap::pheatmap(as.matrix(data_to_plot),cluster_rows = T,
                         cluster_cols = F,
                         show_colnames = T,
                         fontsize_col = 4,
